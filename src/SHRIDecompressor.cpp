...
bool SHRIDecompressor::detectHeaderXPK(uint32_t hdr)
{
	...
	_state(state)
{
	...
	if (!_packedData.read(2,tmp)) return;
	if (tmp<0x80)
	{
		uint16_t size;
		if (!_packedData.readBE(2,size)) return;
		_rawSize=size;
		_startOffset=4;
	} else {
		uint32_t size;
...
bool SHRIDecompressor::isValid() const
{
	return _isValid;
}

bool SHRIDecompressor::verifyPacked() const
{
	return _isValid;
}

bool SHRIDecompressor::verifyRaw(const Buffer &rawData) const
{
	return _isValid;
}
